...
(() => {
  ...
  const stylesheet = `
    ...
    @media print {
      ...
      /* Safari anchors a fixed element's inset properties to the page's
       * MARGIN box at print, not the physical sheet edge the way Blink
       * does — so once @page carries the vertical margin (the wk-print
       * case above), top:0/bottom:0 here land a whole margin's worth
       * further in than intended, overlapping the flow content that
       * starts at that same margin edge. Pulling back by the margin
       * restores the physical-edge anchoring the padding above assumes. */
      .sheet.wk-print:not(.paginated) ::slotted([slot="header"]) {
        top: calc(-1 * var(--doc-page-margin));
      }
      .sheet.wk-print:not(.paginated) ::slotted([slot="footer"]) {
        bottom: calc(-1 * var(--doc-page-margin));
      }
    }
  `;
