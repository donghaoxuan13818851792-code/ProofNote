...
(function (root) {
  ...
  function agentNotice(options) {
    return string(options && options.notice) || "Proofnote Editable HTML v2. Edit only semantic fields and visible metadata; preserve the immutable source carrier, block ID/type/field relationships, and explicit LaTeX templates. To insert a block, copy a compatible block shape, assign a unique ext_ ID, retain a supported type, and provide every required field. CSS, classes, wrappers, syntax highlighting, and rendered KaTeX are visual-only and will be regenerated by Proofnote.";
  }
  ...
  function htmlDocument(document, envelope, source, sourceHash, htmlHash, options) {
    ...
    return "<!doctype html><html" + htmlLanguage + "><head><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width,initial-scale=1\"><meta name=\"proofnote-format\" content=\"editable-html\"><meta name=\"proofnote-version\" content=\"" + VERSION + "\"><meta name=\"proofnote-magic\" content=\"" + MAGIC + "\"><meta name=\"proofnote-document-id\" content=\"" + attr(envelope.documentId) + "\"><meta name=\"proofnote-revision-id\" content=\"" + attr(envelope.revisionId) + "\"><meta name=\"proofnote-html-sha256\" content=\"" + htmlHash + "\"><title>" + escapeHtml(title) + "</title><style>" + safeCss + "</style></head><body><!-- " + agentNotice(options).replace(/-->/g, "—>") + " --><article class=\"pn-document pn-editable-document" + presentationClass + "\" data-pn-document=\"" + MAGIC + "\"><template data-pn-metadata>" + metadata + "</template>" + visibleMetadata + "<main data-pn-blocks>" + blocks + "</main></article>" + sourceMarkup(source, sourceHash) + "</body></html>";
  }
